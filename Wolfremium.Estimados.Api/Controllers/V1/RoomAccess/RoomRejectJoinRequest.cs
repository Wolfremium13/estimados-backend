...
) : ControllerBase
{
    ...
    private async Task<Either<LanguageExt.Common.Error, Unit>> NotifyApplicantRejected(Guid roomId, Guid requestId)
    {
        try
        {
            ...
            if (isDev)
            {
                logger?.LogInformation("Web API: Sending notification OnJoinRequestRejected to room {RoomId} for request {RequestId}.", roomId, requestId);
            }
