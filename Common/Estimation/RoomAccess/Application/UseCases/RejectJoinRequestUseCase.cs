...
public class RejectJoinRequestUseCase(IEstimationRoomRepository roomRepository) : IRejectJoinRequestUseCase
{
    public async Task<Either<Error, Unit>> Execute(RejectJoinRequestCommand command)
    {
        return await (
            ...
            from _ in room.RejectJoinRequest(requestId).ToAsync()
