...
public class EstimationRoom
{
    ...
    public Either<Error, Unit> AddJoinRequest(JoinRequest request)
    {
        if (!IsActive)
        {
            return Either<Error, Unit>.Left(Error.New(new RoomClosedException("Cannot add join request. The room is closed.")));
        }
    ...
    public Either<Error, Unit> ApproveJoinRequest(RequestId requestId)
    {
        if (!IsActive)
        {
            return Either<Error, Unit>.Left(Error.New(new RoomClosedException("Cannot approve request. The room is closed.")));
        }
    ...
    public Either<Error, Unit> RejectJoinRequest(RequestId requestId)
    {
        if (!IsActive)
        {
            return Either<Error, Unit>.Left(Error.New(new RoomClosedException("Cannot reject request. The room is closed.")));
        }
