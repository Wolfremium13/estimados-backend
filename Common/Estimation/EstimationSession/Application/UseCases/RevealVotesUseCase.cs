...
public class RevealVotesUseCase(IEstimationSessionRepository sessionRepository) : IRevealVotesUseCase
{
    public async Task<Either<Error, EstimationSessionDto>> Execute(RevealVotesCommand command)
    {
        return await (
            from sId in SessionId.Create(command.RoomId).ToAsync()
            from session in sessionRepository.FindById(sId).ToAsync()
            from _ in session.RevealVotes().ToAsync()
