...
public class RoomApproveJoinRequestShould
{
    ...
    private readonly ILogger<RoomApproveJoinRequest> _logger = Substitute.For<ILogger<RoomApproveJoinRequest>>();
    ...
    public RoomApproveJoinRequestShould()
    {
        ...
        _controller = new RoomApproveJoinRequest(_useCase, _hubContext, _logger);
    ...
    public async Task ReturnOkOnSuccess()
    {
        ...
        await _useCase.Received(1)
            .Execute(Arg.Is<ApproveJoinRequestCommand>(c => c.RoomId == roomId && c.RequestId == requestId));
    ...
    public async Task ReturnProblemDetailsOnFailure()
    {
        var roomId = Guid.NewGuid();
        var requestId = Guid.NewGuid();
        var error = Error.New(new RoomClosedException("Room is closed"));
